...
xcopy /s /q "%APP_ROOT%encode\src\*.*" "%APP_ANDROID_ROOT%assets\src\"
echo - copy resources
mkdir "%APP_ANDROID_ROOT%assets\res"
xcopy /s /q "%APP_ROOT%encode\res\*.*" "%APP_ANDROID_ROOT%assets\res\"
echo - copy config
copy "%APP_ROOT%config.json" "%APP_ANDROID_ROOT%assets\config.json"
echo - copy updater.zip
copy "%APP_ROOT%encode\updater.zip" "%APP_ANDROID_ROOT%assets\res\updater.zip"
echo - copy flist
copy "%APP_ROOT%encode\flist" "%APP_ANDROID_ROOT%assets\res\flist"
